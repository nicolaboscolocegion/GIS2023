...
public class Plug extends AbstractPlugIn {
  ...
  public boolean execute(PlugInContext context) throws Exception {
    ...
    int reportAltitude = reportSelected.getInteger(
      reportSelected.getSchema().getAttributeIndex("elevation")
    );
    ...
    for (Feature f : staionList) {
      ...
      if (f.getInteger(altitudeIndex) > reportAltitude) {
        if (
          upstreamDistances[0] != -1 &&
          upstreamDistances[0] >=
          (distanceTemp = reportGeo.distance(f.getGeometry()))
        ) {
    ...
    if (upstreamStations[0] != null) {
      ...
      nearestMonitorUnit.setAttribute(
        "altitude",
        upstreamStations[0].getAttribute("altitude")
      );
      nearestMonitorUnit.setAttribute(
        "cod_staz",
        upstreamStations[0].getAttribute("cod_staz")
      );
    ...
    if (upstreamStations[1] != null) {
      ...
      farMonitorUnit.setAttribute(
        "altitude",
        upstreamStations[1].getAttribute("altitude")
      );
      farMonitorUnit.setAttribute(
        "cod_staz",
        upstreamStations[1].getAttribute("cod_staz")
      );
    ...
    if (downstramStation != null) {
      ...
      downStreamMonitorUnit.setAttribute(
        "altitude",
        downstramStation.getAttribute("altitude")
      );
      downStreamMonitorUnit.setAttribute(
        "cod_staz",
        downstramStation.getAttribute("cod_staz")
      );
    ...
    FeatureCollection nearReports = new FeatureDataset(
      reports.getFeatures().get(0).getSchema().clone()
    );

    for (Feature r : reports.getFeatures()) {
      if (
        bufferReport.contains(r.getGeometry()) &&
        !r.getAttribute("id_report").equals(reportSelectedName)
      ) {
    ...
    context
      .getLayerManager()
      .addLayer("Result", "near stations", nearestStations);
